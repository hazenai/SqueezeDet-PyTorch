...
class DatasetWrapper(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image, image_meta, gt_boxes, gt_class_ids = self.dataset.preprocess(image, image_meta, gt_boxes, gt_class_ids)

        print(gt_boxes)


        if  gt_boxes is None:
            gt_boxes = np.array([[100,100,101,101]])
...
def compute_dataset_anchors_seed(dataset, anchors_per_grid=6,
